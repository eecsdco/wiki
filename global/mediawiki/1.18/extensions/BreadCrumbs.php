...
$wgExtensionCredits['other'][] = array(
        ...
        'url' => 'http://www.ehartwell.com/BreadCrumbs',
...
function buildBreadcrumbs( $skin, $template ) {
	...
	//echo "<pre>"; print_r($skin); echo "</pre>"; die();
...
function matchFirstCategory( $menuname, $categories ) {
    ...
    foreach ( $matches[1] as $nav ) {
        ...
        if ( $pos !== false ) {
            ...
            else if ( array_search( $cat, $categories ) !== false ) {
                $breadcrumb = $crumb;
                break;
            }
        }
...
function loadTemplate( $title ) {
    ...
    if ( $article ) {
        ...
        if ( $template ) {
            ...
            $template = preg_replace_callback('/'.DELIM.DELIM.'(.*?)'.DELIM.DELIM.'/',
                                              'var_callback', $template );
...
function normalizeParameters( $input, $delimiter, $count ) {
    ...
    $output = array();
    for ( $i = 0 ; $i < $count ; $i++ ) {
        $output[$i] = ($i < count($params)) ? trim($params[$i]) : '';
        $output[$i] = str_replace("\x07", $delimiter, $output[$i]);
    }
    return( $output );
}
 
# Callback function to translate pseudo-variables in preg_replace_callback 
# $matches[0] is the complete match
function var_callback( $matches )
{
    return( translate_variable( $matches[1] ) );
}
